...
export interface Client {
  ...
  // ── 포털 노출 시작일 ──
  // 이 날짜(YYYY-MM-DD) '이전'의 작업·보고서는 클라이언트 포털에서 숨긴다(내부 시스템은 전부 노출).
  // 정식 서비스 개시 전 테스트 데이터(예: 6월)를 광고주에게 안 보이게 하는 용도.
  // 비우면 DEFAULT_PORTAL_CUTOFF 를 사용. 클라이언트별로 온보딩 시점에 맞춰 조정 가능.
  portalCutoff?: string;
}

// 포털 노출 시작일 기본값 — 정식 서비스 개시일(2026-07-01). 클라이언트별 portalCutoff 미설정 시 이 값을 적용.
// ⚠️ 서버 강제값은 마이그레이션 0043 의 my_portal_cutoff() 폴백과 반드시 동일하게 유지할 것.
export const DEFAULT_PORTAL_CUTOFF = '2026-07-01';
