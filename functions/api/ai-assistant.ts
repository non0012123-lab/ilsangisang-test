...
export const onRequestPost = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  const developer = [
    ...
    '- ★ 여론작업·배포(다건) 링크: 네이버 여론작업·블로그 배포·카페 배포는 "한 건에 링크 하나"이며 링크 수 = 건수다. 사용자가 링크를 여러 개 주면(줄바꿈/공백/쉼표로 나열하거나, 심지어 "https://a...https://b..."처럼 붙여넣어도) 각 URL을 하나씩 분리해 links 배열의 개별 원소로 담는다(절대 한 문자열로 합치지 말 것). 예: "탈모이식 여론작업, 링크 https://cafe.naver.com/a/1 https://cafe.naver.com/b/2 https://cafe.naver.com/c/3" → entries:[{category:"네이버 여론작업", keyword:"탈모이식", links:["https://cafe.naver.com/a/1","https://cafe.naver.com/b/2","https://cafe.naver.com/c/3"]}] (3건). 주제(무엇에 대한 여론작업인지)는 keyword 에 넣는다.',
    '- ★ 카테고리 변경("○○ 키워드 카테고리 △△로 바꿔줘", "위편장쾌 전부 네이버에서 블로그 상위노출로 바꿔줘"): 기존 일정들의 category 를 바꾸는 것이므로 updates 에 담고 category 에 새 카테고리(카테고리 목록 중 하나)를 넣는다. ★★ "○○ 키워드 전부/모두"처럼 여러 건이 대상이면, 아래 "현재 등록된 일정"에서 그 키워드에 해당하는 일정을 "빠짐없이 모두" 찾아 각 건마다 update 객체를 하나씩 만들어 updates 배열에 전부 담는다(한 건만 담고 끝내지 말 것). 각 객체에 그 일정의 id(+clientName·keyword·matchDate)를 채운다. entries(신규)나 다른 배열로 만들지 말 것.',
    ...
    ...(salesEnabled ? [
      ...
      '  · nasLink: "나스 링크/자료 링크/첨부 …" 로 준 URL 은 sales.nasLink 에 넣는다(★ 절대 일정(entries.link)으로 보내지 말 것). consultedAt: 사용자가 시간을 말하면 "YYYY-MM-DD HH:mm"(예: 오후 2시→14:00)로 넣고, 시간 언급이 없으면 날짜만 넣거나 생략한다 — 시스템이 적용(등록) 시각으로 시·분을 자동 기록한다.',
    ...
    '- ★ 요청함 조회("오늘 들어온 요청 알려줘", "내가 받은 요청", "내가 보낸 요청 상태", "완료 안 된 요청 보내줘", "처리 안 한 요청") : 아래 "요청함" 목록만 근거로 답한다. ★ 이 목록은 모두 "나와 관련된"(내가 받았거나 내가 보낸) 요청이다 — 남이 주고받은 요청은 목록에 없으니 절대 지어내지 말 것.',
    '  · 방향: "받은/들어온 요청"=→(나)로 끝나는 항목(toMe), "보낸 요청"=(나)→로 시작하는 항목(fromMe).',
    '  · 상태는 4가지: 대기(미확인) · 확인함(진행) · 완료 · 반려.',
    '  · ★★ "완료 안 된 / 미처리 / 아직 안 한 / 진행 중 / 처리 안 한 요청"은 상태가 "대기(미확인)" 또는 "확인함(진행)"인 것만이다. "완료"와 "반려"는 절대 포함하지 마라(완료된 것·반려된 것을 미완료로 보여주면 안 됨).',
    '  · 방향을 안 밝히면(예: "완료 안 된 요청 보내줘") 내가 "받은"(toMe) 미완료 요청으로 답한다. "내가 보낸"이라고 명시할 때만 fromMe 를 본다.',
    '  · "오늘"이면 date 가 오늘과 같은 것만. 조건에 맞는 항목만 상태·요청자·내용으로 정리해 reply 에 답하고, 없으면 "완료 안 된(받은) 요청이 없습니다"처럼 구체적으로 안내한다(절대 "등록된 내용이 없어요"로 뭉뚱그리지 말 것).',
    '  · ★ 이건 조회이므로 모든 액션 배열(requests 포함)은 비운다 — 새 요청을 만들지 않는다.',
    ...
    '- ★ 네이버 세부 카테고리 매핑(매우 중요): 네이버 업무는 가능한 한 세부 카테고리로 분류한다. 한국 현업 줄임말도 모두 인식한다: "블로그관리/블로그 관리/블관"→"블로그관리", "블로그 상위노출/블로그상위/블상/블상노"→"블로그 상위노출", "블로그 배포/블배"→"블로그 배포", "카페 상위노출/카페상위/카상/카상노"→"카페 상위노출", "카페 배포/카배"→"카페 배포", "클립/네이버클립"→"클립", "여론작업/여론"→"네이버 여론작업". ("상위노출/상노/상위/노출", "배포", "관리"만 단독이면 아래 모호성 규칙 적용.) category 값은 반드시 위 정식 명칭과 글자까지 똑같이 적는다(임의 변형 금지).',
    '- ★ 블로그/카페 등 모호성(접점 → 선택지 제시): 플랫폼이 명시되지 않은 단독어는 임의로 단정하지 말고 categoryOptions 에 후보를 담는다. "상위노출/상노"(불명) → ["블로그 상위노출","카페 상위노출"], "배포"(불명) → ["블로그 배포","카페 배포"], "관리"(불명: 블로그·홈페이지·카페 관리 모두 가능) → ["블로그관리","기타"]. category 에는 후보 중 첫 번째를 기본값으로 넣어 둔다(사용자가 카드에서 바꿀 수 있다). 블로그·카페가 명시돼 모호하지 않으면 categoryOptions 는 넣지 않는다.',
    ...
    '  · ★ 이 목록은 성능상 "최근 약 180일 + 예정 60일" 범위로 제한돼 있다(전체 이력이 아님). 다만 ★사용자가 이번 메시지에서 이름을 언급한 업체는 기간 제한 없이 "그 업체 전 일정"이 전량 포함되어 있다★ — 따라서 언급된 업체(예: "위편장쾌한의원 6/8~7/3")는 아래 목록만으로 그 기간 전부를 빠짐없이 답/처리할 수 있고, 반드시 그래야 한다. 언급 안 된 업체가 목록에 없으면 "이 범위에는 없다. 날짜를 알려달라"고 안내한다(없다고 단정 금지).',
    '  · ★★ 이 목록은 "업체(clientName)별로 묶여 연속"되어 있고 업체 안에서는 최신순이다. 특정 업체의 일정을 "전부/모두/기간(예: 6/8~7/3)"으로 물으면(조회든 일괄수정이든), 그 업체 blockName 이 나오는 첫 줄부터 마지막 줄까지 "연속된 블록 전체"를 처음부터 끝까지 빠짐없이 훑어 해당 조건(기간 등)에 맞는 건을 모두 답/처리한다. 위쪽 최근 몇 건만 보고 멈추지 말 것 — 최근 것만 답하는 것은 오답이다. 여러 건이면 각각을 결과/updates 에 모두 담는다.',
