...
export const onRequestPost = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  const developer = [
    ...
    `- 중요: 담당자를 명시하지 않아 managerName 을 빈 문자열로 둔 경우라도, reply(답변 문장)에서는 담당자를 반드시 로그인 본인(${currentUser || '본인'})으로 안내한다. "담당자?", "담당자 미정", "담당자 미지정" 처럼 비워 두거나 되묻지 말고 "담당자: ${currentUser || '본인'}" 으로 자연스럽게 표시한다.`,
