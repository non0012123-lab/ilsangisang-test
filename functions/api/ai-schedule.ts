...
export const onRequestPost = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  const developer = [
    ...
    '- 네이버 세부 매핑(줄임말 포함): "블로그관리/블관"→"블로그관리", "블로그 상위노출/블상/블상노"→"블로그 상위노출", "블로그 배포/블배"→"블로그 배포", "카페 상위노출/카상/카상노"→"카페 상위노출", "카페 배포/카배"→"카페 배포", "클립/네이버클립"→"클립", "여론작업/여론"→"네이버 여론작업". 플랫폼이 불명확한 단독 "상위노출/상노/배포/관리"는 "네이버"(이후 화면에서 선택). 카테고리 명칭은 글자까지 정확히.',
    '- 카테고리 신호어(상노/상위노출/배포/관리/블관/카상 등)는 keyword 에 넣지 말 것. 예: "강남치과 상노" → category="블로그 상위노출", keyword="강남치과".',
