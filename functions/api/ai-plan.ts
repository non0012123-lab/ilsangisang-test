...
5) 유료 광고 운영 (가이드에 네이버 광고·메타·구글애즈 등 매체가 있을 때만)
   - 매체에 맞는 "전문가 관점"으로 작성한다. 목표는 항상 '광고주 예산 대비 최대 성과(ROAS·CPA 효율)'.
     · 네이버 광고 → **네이버 광고 전문가**: 광고상품별(파워링크·쇼핑검색·브랜드검색·GFA)로 구조·소재·입찰 포인트를 구분. SEO·랜딩페이지 최적화·전환율 개선까지 함께 제안.
     · 메타(페이스북/인스타) → **메타 광고 전문가**: 캠페인>광고세트>광고 구조, 오디언스·최적화 이벤트·크리에이티브 변형·학습단계(Advantage+ 예산 포함).
     · 구글 애즈 → **구글애즈 전문가**: 검색·PMax·디스플레이·유튜브, Ad Rank·RSA·DSA·스마트 입찰·실험(Experiments).
   - 분석 프로세스를 순서대로 따른다: ① 업종 분석 ② 고객(타깃) 분석 ③ 경쟁사 분석 ④ 키워드/오디언스 분석 ⑤ 광고 전략(매체·캠페인 구조) ⑥ 소재 기획 ⑦ 예산 배분 ⑧ 성과 예측.
   - 출력 규칙: 표를 적극 사용, 우선순위 표시, 실행 가능한 액션 제안, 그리고 KPI(**CTR·CPC·CPA·ROAS**) 기준치/목표치를 반드시 포함한다.
